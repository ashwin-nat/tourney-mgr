...
import { createBalancedGroups, generateGroupStageMatches, maybeStartKnockoutAfterGroups, } from "../formats/groups";
import { generateKnockoutRoundOne, maybeGenerateNextKnockoutRound, } from "../formats/knockout";
...
function runFormatProgression(tournament) {
    let next = tournament;
    ...
    if (!hasUnplayed && next.matches.length > 0)
        next.status = "COMPLETED";
    return next;
}
function applyMatchSimulation(tournament, matchIds) {
    const set = new Set(matchIds);
    const matches = tournament.matches.map((match) => {
        if (!set.has(match.id) || match.played)
            return match;
...
export const useTournamentStore = create((set, get) => {
    ...
    return {
        ...
        createTournament(input) {
            const participants = validateParticipants(input.participants.map((p) => ({ ...p, id: p.id || makeId("p") })));
            const tournament = {
                id: makeId("t"),
        ...
        deleteTournament(id) {
            ...
            const currentTournamentId = get().currentTournamentId === id ? tournaments[0]?.id ?? null : get().currentTournamentId;
        ...
        updateParticipantRating(tournamentId, participantId, rating) {
            const tournaments = get().tournaments.map((t) => {
                ...
                return {
                    ...t,
                    participants: t.participants.map((p) => p.id === participantId ? { ...p, rating: clampRating(rating) } : p),
                };
            });
        ...
        simulateMatch(id, matchId) {
            const tournaments = get().tournaments.map((t) => t.id === id ? applyMatchSimulation(t, [matchId]) : t);
            const next = { tournaments, currentTournamentId: get().currentTournamentId };
            persist(next);
            set({ tournaments });
        },
