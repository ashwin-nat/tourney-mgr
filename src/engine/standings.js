...
export function buildStandings(participants, matches) {
    const table = {};
    ...
    for (const p of participants) {
        ...
        table[p.id].buchholz = opponents.reduce((acc, id) => acc + (table[id]?.points ?? 0), 0);
    }
    return table;
}
export function rankParticipants(participants, standings) {
